...
export default Vue.extend({
  ...
  methods: {
    ...
    getExportedData(event) {
      this.offer[event.group] = event.data.target;
      this.updateOffer();
    },

    updateOffer(back?: boolean) {
      this.offer.startDate =
        this.time.startDate && this.time.startTime
          ? `${this.time.startDate} ${this.time.startTime}`
          : null;
      this.offer.expirationDate =
        this.time.expirationDate && this.time.expirationTime
          ? `${this.time.expirationDate} ${this.time.expirationTime}`
          : null;
      this.multiplyTen();
    ...
    changeNumber(number: number | undefined, operator: OperatorEnum) {
